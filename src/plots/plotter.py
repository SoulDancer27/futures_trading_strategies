...
def prepare_plot_data(
    results_list: List[Tuple[str, BacktestResult]], 
    realized_vol_window: int = 21
) -> Dict[str, Dict[str, pd.Series]]:
    """
    Pre-calculates all necessary series for plotting.
    ...
    for name, res in results_list:
        plot_data[name] = {
            ...
            'realized_vol': res.realized_vol,  # <--- JUST READ IT
...
# ==========================================
def _render_equity(ax, data, names, colors, plot_pct=False):
...
def _render_drawdown(ax, data, names, colors):
    for name, color in zip(names, colors):
...
def _render_leverage(ax, data, names, colors):
    for name, color in zip(names, colors):
...
def _render_price(ax, data, names, colors):
...
def plot_backtest_results(
    ...
    # 4. Prepare Data (Calculated once)
    plot_data = prepare_plot_data(results_list, realized_vol_window)
